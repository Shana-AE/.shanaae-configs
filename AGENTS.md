...
│   │   ├── for-tools/ # Symlinked skills for AI tools (generated, gitignored)
│   │   └── link-skills.sh
...
```bash
# Link all skills to for-tools directory (mandatory after clone — for-tools is gitignored)
...
| `private/` | Machine-local skills — **gitignored, never committed to remote**; still linked into `for-tools/` so all AI tools pick them up | backend-api-generator-skill |
| `for-tools/` | Symlinked skills for AI tools (**generated by `link-skills.sh`, gitignored**) | — |
...
4. Run `bash ai/skills/link-skills.sh` to link (skills from `private/` are linked the same way but never reach the remote repo)
...
```bash
# Re-link skills (for-tools/ is gitignored — empty until this runs)
bash ai/skills/link-skills.sh
