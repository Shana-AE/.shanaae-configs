...
This is the central configuration hub for AI development tools (Claude Code, OpenCode, Cursor, Qoder, Codex CLI). It manages skills, MCP servers, and user rules across multiple AI coding agents.
...
- **Active Tools**: Claude Code, OpenCode, Cursor, Qoder (Qoder + QoderCN), Codex CLI, dsh (DeepSeek Harness), Pi
...
│   ├── skills -> ../ai/skills/for-tools/cursor
│   ├── AGENTS.md       # Cursor global instructions
│   └── user_rules -> ../ai/user_rules
├── qoder/              # Qoder / QoderCN configuration
│   ├── skills -> ../ai/skills/for-tools/qoder
│   ├── AGENTS.md       # Qoder global instructions
│   └── user_rules -> ../ai/user_rules
├── .local/            # Runtime data (gitignored)
├── ai/
│   ├── mcp/           # MCP server configs (.example templates)
...
│   │   ├── for-tools/ # Per-agent pools: opencode/ claude/ codex/ pi/ cursor/ qoder/ (generated, gitignored)
│   │   └── link-skills.sh
...
# Sync MCP server config + enabled/disabled status across opencode, ~/.claude.json, .claude/mcp.json, ~/.codex/config.toml, and cursor/qoder mcp.json
...
| `for-tools/<agent>/` | Per-agent symlink pool (opencode/ claude/ codex/ pi/ cursor/ qoder/); **generated by `link-skills.sh`, gitignored** | — |
...
| Cursor | `~/.cursor/mcp.json` — live, written by `sync_mcp.py` | `{env:VAR}` resolved | `disabled: true` |
| Qoder / QoderCN | `~/.qoder/mcp.json`, `/mnt/c/Users/shana/.qoder/mcp.json`, `/mnt/c/Users/shana/.qoder-cn/mcp.json` — live, written by `sync_mcp.py` | `{env:VAR}` resolved | `disabled: true` |
...
# Cursor / Qoder (WSL) — Windows-side pools are copy-synced by sync_cursor_qoder.py
~/.cursor/skills -> configs/cursor/skills
~/.qoder/skills -> configs/qoder/skills
