...
## OpenCode from WSL — how to invoke (read this first on this host)

This host runs opencode inside WSL; the driver is a Windows binary. `cua-driver`
on PATH is a symlink to the Windows exe (WSL interop). **Prefer the wrapper**
`scripts/cua.sh` in this skill directory: it auto-starts the Windows daemon
(`autostart kick` / `serve`) if it is down, then runs the one-shot CLI call.

- One-shot tool call: `scripts/cua.sh <tool> '<json-args>'` (e.g. `get_screen_size '{}'`)
- Management: `scripts/cua.sh status` / `serve` / `list-tools` / `describe <tool>`
- Plain `cua-driver <tool> '<json>'` also works **only** when the daemon is already
  running (one-shot CLI calls use the daemon service path).
- The daemon auto-starts at every Windows logon (Scheduled Task `cua-driver-serve`,
  registered via `cua-driver autostart enable`). If `status` says not running, run
  `scripts/cua.sh serve` or `cua-driver autostart kick`.
