...
AgentMemory is deployed on the NAS (`192.168.86.62:3111`) via Docker. It captures coding session events automatically. The MCP server (`@agentmemory/mcp`) provides tools for structured memory operations.
...
Use `memory_save` when encountering:
...
- Use the **context7 skill** (curl to context7.com API with `CONTEXT7_API_KEY`) when you need to search documentation
